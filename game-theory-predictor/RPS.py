...
my_moves = ["R"]  # Start with an initial move
opponent_history = []
strategy = [0, 0, 0, 0]
opponent_guess = ["", "", "", ""]
strategy_guess = ["", "", "", ""]
...
def player(prev_play):
    ...
    # Ensure there's enough history for prediction
    ...
    # Ensure there are at least 2 of my moves for prediction
    if len(my_moves) >= 2:
...
def predict_move(history, n, play_order):
    # Ensure that the agent is not accessing out-of-range indices in history
    if len(history) < n:
        return "R"  # Default to Rock if there isn't enough history
